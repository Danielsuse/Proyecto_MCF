...
# Add a legend and labels to make the chart more informative
     ...
     # Función para calcular el VaR y ES para una rolling window
     def var_y_es_rolling_window(df_rendimientos, window_size = 252, confidence_levels = [0.95, 0.99]):
    ...
    #bucle para seleccionar una ventana movildel tamaño definido en window_size,i.e, 252
          while j <= len(df_rendimientos) - 1:
              rw = df_rendimientos.iloc[i:j]
              mean_rw = rw["Close"].mean() #media de la ventana en cuestión
              std_rw = rw["Close"].std() #sd de la ventana en cuestión
        ...
        # Calculo del VaR parámetrico e Histórico, misma metodología anteriormente definida adaptada a las rolling window
              for level in confidence_levels:
                  ...
                  hVaR = round(rw["Close"].quantile(1 - level), 4)
                  VaR_h[level].append(hVaR)
                  ES[level].append(round(rw["Close"][rw["Close"] <= hVaR].mean(), 4)) # Cálculo del Expected Shortfall
    ...
    #regresa un dataframe creado a partir de la recolección de datos del diccionario result.
          return pd.DataFrame(result, index=df_rendimientos["Close"].iloc[window_size:].index)

# Llamada a la función para calcular VaR y ES con rolling window
     rolling_window_results = var_y_es_rolling_window(df_rendimientos)

# Unimos resultados con los daily_returns
     final = df_rendimientos.iloc[252:].join(rolling_window_results)
